...
- **DevFleet MCP server** — Claude DevFleet MCP for parallel agent orchestration. Look up the official source via `grep -ri "devfleet" ~/.claude/plugins/marketplaces/ 2>/dev/null | head -10` or check `~/.claude/mcp.json` if configured. **Verify install command before publishing**; if DevFleet is author's private MCP not yet public, this command is unusable without that infra.
- The command orchestrates parallel Claude Code agents via the DevFleet MCP; reader without DevFleet installed cannot use this command directly
